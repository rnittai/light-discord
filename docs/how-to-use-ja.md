...
MVP の実装では、ダウンスケーリングされた JPEG フレームを、既存の TCP JSON コントロール接続上で base64 エンコードして送信しています。これは友達やセルフホスト環境での検証用であり、本番グレードのビデオではありません。今後のプロダクション対応では、専用のバイナリ/ビデオトランスポートに移行し、コーデック、レート制御、暗号化を改善する予定です。
